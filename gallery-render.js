...
let _openModalByKey = () => {};
let _addCardToDeck = () => {};
let _removeCardFromDeck = () => {};
let _getCardDeckCount = () => 0;
let _isDeckPanelOpen = () => false;
let _persistPreferences = () => {};

export function initRenderCallbacks({
  openModalByKey,
  addCardToDeck,
  removeCardFromDeck,
  getCardDeckCount,
  ...
  if (addCardToDeck) _addCardToDeck = addCardToDeck;
  if (removeCardFromDeck) _removeCardFromDeck = removeCardFromDeck;
  if (getCardDeckCount) _getCardDeckCount = getCardDeckCount;
  if (isDeckPanelOpen) _isDeckPanelOpen = isDeckPanelOpen;
  if (persistPreferences) _persistPreferences = persistPreferences;
}
...
function loadIllustratorCell(card, cell) {
  ...
  fetch(card.transcriptPath)
    ...
    .then(text => {
      const t = text.trim();
      sharedState.transcriptCache.set(card.key, t);
      if (cell.isConnected) {
...
export function scheduleStackActiveUpdate() {
...
export function getPageCards(totalCards) {
  if (paginationState.mode === "infinite") {
    const count = Math.min(paginationState.infiniteLoadedCount, totalCards);
    return sharedState.filteredCards.slice(0, count);
  }
  const start = (paginationState.currentPage - 1) * paginationState.pageSize;
  return sharedState.filteredCards.slice(start, start + paginationState.pageSize);
}

export function renderPaginationControls(totalCards) {
...
function goToNextPage() {
  const totalPages = Math.ceil(sharedState.filteredCards.length / paginationState.pageSize);
...
function loadMoreInfiniteCards() {
  if (paginationState.infiniteLoadedCount >= sharedState.filteredCards.length) return;

  const startIndex = paginationState.infiniteLoadedCount;
  const newCount = Math.min(startIndex + paginationState.pageSize, sharedState.filteredCards.length);
  const newCards = sharedState.filteredCards.slice(startIndex, newCount);
  ...
  if (displayState.groupBy === "none") {
    ...
    if (wrapper) {
      ...
      renderPaginationControls(sharedState.filteredCards.length);

      if (paginationState.infiniteLoadedCount < sharedState.filteredCards.length) {
        setupInfiniteScroll();
      }
      return;
    }
...
export function renderGallery() {
  ...
  const totalCards = sharedState.filteredCards.length;
...
export function renderActiveFilters(parsedQuery) {
...
export function createTagChipElement(tag, className = "card-tag") {
...
export function createCardElement(card, index = 0) {
  ...
  const deckCount = _getCardDeckCount(card.key);
  ...
  deckOverlay.addEventListener("click", (event) => {
    ...
    if (!btn) return;
    if (btn.dataset.action === "inc") _addCardToDeck(card.key);
    else if (btn.dataset.action === "dec") _removeCardFromDeck(card.key);
  });
  ...
  cardButton.addEventListener("click", () => {
    if (_isDeckPanelOpen()) return;
    _openModalByKey(card.key, true);
  });
  return cardButton;
}
...
function createListCardElement(card) {
  ...
  deckEl.addEventListener("click", (event) => {
    ...
    if (btn.dataset.action === "inc") {
      _addCardToDeck(card.key);
    } else if (btn.dataset.action === "dec") {
      _removeCardFromDeck(card.key);
    }
    const newCount = _getCardDeckCount(card.key);
